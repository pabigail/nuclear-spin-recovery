...
def make_df_from_Ivady_file_for_exp(file_path, strong_thresh, weak_thresh):
    '''
    input:
        file_path (string): location of text file with hyperfine couplings (MHz) and locations (A) 
        of nuclear spins
    ...
    hf_data['A_yz'] *= MHz_to_KHz
    hf_data['A_perp'] = np.sqrt(hf_data['A_xz']**2 + hf_data['A_yz']**2)
    hf_data['A_par'] = -1*hf_data['A_zz'] # to match exp sign convention
    ...
    # to match taminiau data convention
    most_common_trans = (-3.78, 0.73, 8.6)
    hf_df['x'] = hf_df['x'] - most_common_trans[0]
    hf_df['y'] = hf_df['y'] - most_common_trans[1]
    hf_df['z'] = hf_df['z'] - most_common_trans[2]
...
def make_exp_params_dict(num_exps, num_pulses, mag_field, noise, timepoints, T2=None):
    '''
    input:
    ...
    if T2 == None:
        T2 = [1 for _ in range(len(mag_field))]
        
    if is_array_like(T2):
        exp_params['T2'] = np.array(T2)
    else:
        exp_params['T2'] = np.array([T2])
...
def calculate_coherence_with_T2(spin_list, hf_df, exp_params):
    '''
    input:
        spin_list (1d numpy array of ints): indices of spins in hf_df
        hf_df (dataframe): hyperfine couplings and locations of all nuclear spins
    ...
    coherence_signals = calculate_coherence(spin_list, hf_df, exp_params)
    coherence_signals_with_T2 = []
    
    for index in range(exp_params['num_experiments']):
        T2 = exp_params['T2'][index]
        ...
        coherence_signals_with_T2.append(coherence_with_T2)
    
    return coherence_signals_with_T2


def calculate_coherence_with_T2_and_noise(spin_list, hf_df, exp_params):
    '''
    input:
        spin_list (1d numpy array of ints): indices of spins in hf_df
        hf_df (dataframe): hyperfine couplings and locations of all nuclear spins
    ...
    for index in range(exp_params['num_experiments']):
        ...
        neg_exp = [np.exp(-t/T2) for t in time]
        coherence_with_T2 = [L*e for L, e in zip(coherence_signals[index], neg_exp)]
        coherence_with_T2 = [L + np.random.normal(0, exp_params['noise'][index]) 
                             for L in coherence_with_T2]
        coherence_signals_with_T2.append(coherence_with_T2)
...
def get_error_spin_data(coherence_data, spin_list, exp_params, hf_df):
    ...
    simulated_coherence_signals = calculate_coherence_with_T2(spin_list, hf_df, exp_params)
    
    err = 0
    for index in range(exp_params['num_experiments']):
        err += np.linalg.norm(np.array(coherence_data[index]) - np.array(simulated_coherence_signals[index]))
...
# def within_model_step_RWMH_spins(current_spins, r, hf_df, hf_dist_mat, 
...
#     proposed_spins = get_proposal_spins_within_model_step_RWMH_spins(current_spins, hf_dist_mat, r)
    
#     log_a = get_log_accept_prob_within_model_step_RWMH_with_T2(current_spins, proposed_spins, exp_params,
...
def within_model_step_RWMH_one_spin(current_spins, r, hf_df, hf_dist_mat, 
    ...
    log_a = get_log_accept_prob_within_model_step_RWMH_spin(current_spins, proposed_spins, exp_params,
                                                 coherence_data, r, hf_df, hf_dist_mat, sigma_sq,
                                                       beta_k)
    u = np.random.uniform(0, 1)
    if np.log(u) < log_a:
...
def proposal_step_T2(current_exp_params, index, r_T2=0.1):
    # implementing a symmetric Gaussian proposal distribution with reflection at the boundaries
    
    current_T2 = current_exp_params['T2'][index]
    
    T2_proposal = current_T2 + np.random.normal(0, r_T2)
    
    # make sure 0 < T2 <= 1
    if T2_proposal > 1:
        T2_proposal = 2 - T2_proposal
    elif T2_proposal <= 0:
        T2_proposal = np.abs(T2_proposal + 1e-8)
        
    proposal_exp_params = copy.deepcopy(current_exp_params)
    proposal_exp_params['T2'][index] = T2_proposal
    
    return proposal_exp_params



# TO DO
def within_model_step_RWMH_T2(current_spins, r_T2, exp_index, hf_df, coherence_data, 
                              exp_params, sigma_sq=None, beta_k=None):
    
    if beta_k == None:
        beta_k = 1
        
    proposed_exp_params = proposal_step_T2(exp_params, exp_index, r_T2)
    
    log_a = get_log_accept_prob_within_model_step_RWMH_T2(current_spins, exp_params, coherence_data,
                                                          proposed_exp_params, hf_df, 
                                                          sigma_sq=sigma_sq, beta_k=beta_k)
    
    u = np.random.uniform(0, 1)
    if np.log(u) < log_a:
        next_exp_params = proposed_exp_params
    else:
        next_exp_params = exp_params
        
    error = get_error_spin_data(coherence_data, current_spins, next_exp_params, hf_df)
                                                              
    return next_exp_params, error
...
# def get_proposal_spins_within_model_step_RWMH_spins(current_spins, hf_dist_mat, r):
#     '''
#     input:
#         current_spins (1d numpy array of ints of length current_k): indices of current spins
#         hf_dist_mat (2d numpy array of floats): distance (A) between spins i and j
...
def get_log_likelihood_of_spins_given_data_with_T2(spin_list, coherence_data, hf_df, exp_params, sigma_sq=None):
    '''
    input:
    ...
    for index in range(exp_params['num_experiments']):
        ...
        log_likelihood += np.sum(-1*np.square(np.array(coherence_data[index])-
...
def birth_step(current_k, current_spins, r, hf_df, hf_dist_mat, 
                           coherence_data, exp_params, k_max, sigma_sq=None):
    '''
    input:
    ...
    log_a = get_log_accept_prob_jump_step(current_spins, proposed_spins, exp_params,
                                          coherence_data, hf_df, k_max, sigma_sq)
...
# Add T2, do we need two separate functions, one for T2 proposal and one for spin_proposal,
 # update to make sure coherence signal calculated with T2
def get_log_accept_prob_within_model_step_RWMH_spin(current_spins, proposed_spins, exp_params,
    ...
    log_L_prop_spins = get_log_likelihood_of_spins_given_data_with_T2(proposed_spins, coherence_data, 
                                                              hf_df, exp_params, sigma_sq=sigma_sq)
    log_L_curr_spins = get_log_likelihood_of_spins_given_data_with_T2(current_spins, coherence_data, 
                                                              hf_df, exp_params, sigma_sq=sigma_sq)
...
def get_log_accept_prob_within_model_step_RWMH_T2(current_spins, current_exp_params, 
                                                       coherence_data, proposed_exp_params,
                                                       hf_df, sigma_sq=None, beta_k=1):
    
    log_L_prop_T2 = get_log_likelihood_of_spins_given_data_with_T2(current_spins, coherence_data,
                                                                   hf_df, proposed_exp_params,
                                                                   sigma_sq)
    log_L_curr_T2 = get_log_likelihood_of_spins_given_data_with_T2(current_spins, coherence_data,
                                                                   hf_df, current_exp_params,
                                                                   sigma_sq)
   
    
    log_a = min((0, (beta_k*log_L_prop_T2 - beta_k*log_L_curr_T2)))                                                             
    return log_a
...
def parallel_tempering_steps(current_spins, hf_df, hf_dist_mat, exp_params, coherence_data, 
                             num_strands, num_steps, r, beta, sigma_sq=None):
    '''
    input:
    ...
    while count < num_steps:
        ...
        log_L_spins_k1 = get_log_likelihood_of_spins_given_data_with_T2(spins_k1, coherence_data, 
                                                                   hf_df, exp_params, sigma_sq)
        log_L_spins_k2 = get_log_likelihood_of_spins_given_data_with_T2(spins_k2, coherence_data, 
                                                                   hf_df, exp_params, sigma_sq)
